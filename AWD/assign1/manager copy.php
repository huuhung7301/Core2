...
<body>
	...
	<form method="post" action="manager.php">
		<fieldset>
			...
			<p>
				<label>Search for particular burger:</label>
			</p>
			<p>
            	<input type="checkbox" id="product1" name="productSearch[]" value="burg1">
                <label for="product1">The Impossible Lawless Burger</label>
            </p>
            <p>
            	<input type="checkbox" id="product2" name="productSearch[]" value="burg2">
                <label for="product2">The Lawless Truffle Burger</label>
            </p>
			<p>
            	<input type="checkbox" id="product3" name="productSearch[]" value="burg3">
                <label for="product3">The Sunnyside Lawless Burger</label>
            </p>
      			<p>
            	<input type="checkbox" id="product4" name="productSearch[]" value="burg4">
                <label for="product4">The Lawless BBQ Bacon Burger</label>
            </p>
			<p>
            	<input type="checkbox" id="product5" name="productSearch[]" value="burg5">
                <label for="product4">The Lawless Patty Melt</label>
            </p>
			<p>
        		...
        		if (!$conn){
	        		...
	        		if (isset($_POST['productSearch'])){
        				...
        				if ($condition != "")
        					...
        					$condition .= " (";
	        			if (in_array('burg1', $products)){
	        				if ($product_list != "")
	        					$condition .= " OR ";

	        				$condition .= "purchases LIKE '%The Impossible Lawless Burger%'";
	        				$product_list .= "burg1";
	        			}
	        			if (in_array('burg2', $products)){
	        				if ($product_list != "")
	        					$condition .= " OR ";

	        				$condition .= "purchases LIKE '%The Lawless Truffle Burger%'";
	        				$product_list .= "burg2";
	        			}
	        			if (in_array('burg3', $products)){
	        				if ($product_list != "")
	        					$condition .= " OR ";

	        				$condition .= "purchases LIKE '%The Sunnyside Lawless Burger%'";
	        				$product_list .= "burg3";
	        			}
	        			if (in_array('burg4', $products)){
	        				if ($product_list != "")
	        					$condition .= " OR ";

	        				$condition .= "purchases LIKE '%The Lawless BBQ Bacon Burger%'";
	        				$product_list .= "burg4";
	        			}
								if (in_array('burg5', $products)){
	        				if ($product_list != "")
	        					$condition .= " OR ";

	        				$condition .= "purchases LIKE '%The Lawless Patty Melt%'";
	        				$product_list .= "burg5";
	        			}
	        			$condition .= ")";
	        		}
        	...
        	if (!$conn){
        		...
        		if ($_POST["bestSelling"] == "yes" || $_POST["customerPurchase"] == "yes" || $_POST["avgProfit"] == "yes" || $_POST["statusNumber"] == "yes"){
        			...
        			if (!$result) {
        				...
        				$burg1Count = 0; $burg2Count = 0; $burg3Count = 0; $burg4Count = 0;	$burg5Count = 0;	//for best selling burger
        				...
        				while ($record = mysqli_fetch_assoc ($result) ){					//fetch all the records
        					...
        					if ($_POST["bestSelling"] == "yes"){
		        				if (strpos($record["purchases"], "The Impossible Lawless Burger") !== false)		//if burg1 is selected
		        					$burg1Count++;
		        				if (strpos($record["purchases"], "The Lawless Truffle Burger") !== false)			//if burg2 is selected
		        					$burg2Count++;
		        				if (strpos($record["purchases"], "The Sunnyside Lawless Burger") !== false)			//if burg3 is selected
		        					$burg3Count++;
		        				if (strpos($record["purchases"], "The Lawless BBQ Bacon Burger") !== false)		//if burg4 is selected
		        					$burg4Count++;
										if (strpos($record["purchases"], "The Lawless Patty Melt") !== false)		//if burg5 is selected
			        				$burg5Count++;
			        		}
        				...
        				if ($_POST["bestSelling"] == "yes"){
        					$max = $burg1Count; $name = "The Impossible Lawless Burger";
        					if ($burg2Count > $max){
        						$max = $burg2Count;
        						$name = "The Lawless Truffle Burger";
        					}
        					if ($burg3Count > $max){
        						$max = $burg3Count;
        						$name = "The Sunnyside Lawless Burger";
        					}
        					if ($burg4Count > $max){
        						$max = $burg4Count;
        						$name = "The Lawless BBQ Bacon Burger";
        					}
									if ($burg5Count > $max){
        						$max = $burg5Count;
        						$name = "The Lawless Patty Melt";
        					}
